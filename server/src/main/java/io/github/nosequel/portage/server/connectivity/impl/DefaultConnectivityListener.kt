...
class DefaultConnectivityListener : ConnectivityListener {
    ...
    override fun handleConnect(session: Session,  server: Server) {
        this.broadcastStaffMessage("${ChatColor.BLUE}[Staff] ${ChatColor.AQUA}${session.getDisplayName()} ${ChatColor.GREEN}joined ${ChatColor.AQUA}the network (from ${server.name})")
    }
    ...
    override fun handleDisconnect(session: Session, server: Server) {
        this.broadcastStaffMessage("${ChatColor.BLUE}[Staff] ${ChatColor.AQUA}${session.getDisplayName()} ${ChatColor.RED}left ${ChatColor.AQUA}the network (from ${server.name})")
    }
    ...
    override fun handleSwitch(session: Session, serverTo: Server, serverFrom: Server) {
        this.broadcastStaffMessage("${ChatColor.BLUE}[Staff] ${ChatColor.AQUA}${session.getDisplayName()}${ChatColor.GREEN} joined ${ChatColor.AQUA}${serverTo.name} (from ${serverFrom.name})")
    }
